...
![|500](https://img.czhread.asia/img/202212201900477.jpg)
...
## 循环神经网络
RNN三种重要设计模式
- **每个时间步都有输出，并且隐藏单元之间有循环连接的循环网络**![|450](https://img.czhread.asia/img/202212201901786.jpg)
...
- **每个时间步都产生一个输出，只有当前时刻的输出到下个时刻的隐藏单元之间有循环连接的循环网络**![|450](https://img.czhread.asia/img/202212201903089.jpg)
...
![|450](https://img.czhread.asia/img/202212201904260.jpg)
导师驱动过程的最初动机为在缺乏隐藏到隐藏连接的模型中避免通过时间反向传播BPTT
但只要隐藏单元成为较早时间步的函数，BPTT应与导师驱动同时使用

- **隐藏单元之间存在循环连接，但读取整个序列后产生单个输出的循环网络**!![|450](https://img.czhread.asia/img/202212201910958.jpg)
### RNN梯度计算
![|450](https://img.czhread.asia/img/202212201911672.jpg)
- $a^{t}=b+Wh^{t-1}+Ux^{t}$【仿射】
- $h^{t}=tanh(a^{t})$【激活】
- $o^{t}=c+Vh^{t}$【隐藏-输出】
- $\hat{y}^{t}=softmax(o^{t})$【转换输出】
...
![|400](https://img.czhread.asia/img/202212201912543.jpg)
...
- 作为初始状态$h^{0}$
- 二者结合
![|400](https://img.czhread.asia/img/202212201913147.jpg)
![|400](https://img.czhread.asia/img/202212201914001.jpg)
...
![|400](https://img.czhread.asia/img/202212201915075.jpg)
...
![|450](https://img.czhread.asia/img/202212201916954.jpg)
...
![|450](https://img.czhread.asia/img/202212201917437.jpg)
...
![|400](https://img.czhread.asia/img/202212201919464.jpg)

## 长期依赖挑战
结果多阶段传播后的梯度倾向于消失（多数）或爆炸
循环网络涉及相同函数的多次组合，可能导致极端非线性行为
![|450](https://img.czhread.asia/img/202212201919056.jpg)
...
![|450](https://img.czhread.asia/img/202212201921872.jpg)
...
$u_{i}^{t}=\sigma(b_{i}^{u}+\sum\limits_{j}U_{i,j}^{u}x_{j}^{t}+\sum\limits_{j}W_{i,j}^{u}h_{j}^{t-1})$更新门![[专业课/数据分析/附件/Pasted ![|300](https://img.czhread.asia/img/202212201922956.png)
$r_{i}^{t}=\sigma(b_{i}^{r}+\sum\limits_{j}U_{i,j}^{r}x_{j}^{t}+\sum\limits_{j}W_{i,j}^{r}h_{j}^{t-1})$复位门
![|300](https://img.czhread.asia/img/202212201922764.png)
