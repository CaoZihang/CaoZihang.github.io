...
​		通过检索自因话语和互因话语，为它们配置识别情感因果的权重。其中自因话语的检索得分为$scores^{intra}_{i,j}=\frac{[f_q(h_i)(f_k(h_i)+f_e(Clue_j^{intra}))]mask_{i,j}^{intra}}{\sqrt{d_h}} $，其中$f_q(x),f_k(x),f_e(x) $是线性变换，$mask^{intra}_{i,j} $保证目标语句$h_i$检索的语句都是同一讲述人的内因线索，且符合时间顺序。
