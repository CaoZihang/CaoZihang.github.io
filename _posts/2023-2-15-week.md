...
​		$mask_{i,j}^{intra}=\{\begin{aligned} &1,j\leq i\ and\ \phi(h_i)=\phi(h_j)\\\  &0,else\end{aligned}$，其中$\phi(x)$是把语句索引对应到讲述人的映射。互因语句的得分与之类似，$scores_{i,j}^{inter}=\frac{[f_q(h_i)(f_k(h_j)+f_e(Clue_j^{inter}))]mask_{i,j}^{inter}}{\sqrt{d_h}}$。之后通过Softmax函数获取两类语句的权重$\alpha_{i,j}^{joint}=softmax(scores_{i,j}^{intra}+scores_{i,j}^{inter})$。
