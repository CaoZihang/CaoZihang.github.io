...
采用<span style="color: rgba(0, 0, 0, 0.85)"><span style="background-color: rgb(255, 255, 255)">消费 - 闲暇可分离的 CRRA 效用函数，消费带来的正效用与闲暇带来的正效用相互独立</span></span>
...
$\widetilde{Q}_t \widetilde{F}_t = \left( \frac{1 + i_{\text{RoW}, t-1}}{1 + \widetilde{\pi}_t} \right) \widetilde{Q}_t \widetilde{F}_{t-1} + \Delta_{t-1} \frac{P_{t-1}}{P_t} + \left( \frac{P_{H,t}}{P_t} C_{H,t}^* - \frac{P_{F,t}}{P_t} C_{F,t} \right) + \left( \frac{P_{H,t}}{P_t} \widetilde{C}_{H,t} - \frac{P_{\text{RoW},t}}{P_t} C_{\text{RoW},t} \right)$
...
$i_{RoW,t}=1+i_{RoW,t}=(1+\tilde{\iota})+\zeta\left\{exp[(1-\varpi)(nfd_t-\overline{nfd})+\varpi(nfd_t^\ast-\overline{nfd}^\ast)]-1\right\}$
