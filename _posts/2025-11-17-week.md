...
<blockquote style="border-color: #FFB2B2 !important;"> POC: 很奇怪的设定，既然是总误差，就应该把政策设定作为一个参数；现在在定义中使用$\lambda^{true}$后面又用$\forall \lambda$非常混乱</blockquote>

> 这段推导感觉有问题，若严格按照前文的定义$c(x^\ast(\lambda),\lambda^{true})$与$c(x^\ast(\lambda),\lambda)$消不了
